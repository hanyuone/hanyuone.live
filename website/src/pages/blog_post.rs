...
use crate::{
    components::{blog::to_read_time, blog_post::comments::CommentToggle},
    context::BlogContext,
};

pub type BlogSlug = String;

#[derive(Params, PartialEq)]
pub struct BlogPostParams {
    slug: Option<BlogSlug>,
}
...
pub fn BlogPostPage() -> impl IntoView {
    ...
    let post_contents = {
        let slug = slug.clone();
        Resource::new(move || slug.clone(), get_blog_post_ron)
    };

    view! {
        <Title text={front_matter.title.clone()} />
        <div class="flex flex-col items-center">
            <div class="flex flex-col p-4 text-center border-b-[1px]">
